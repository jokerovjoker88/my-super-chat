...
async function boot() {
    try {
        await db.connect();
        await db.query(`CREATE TABLE IF NOT EXISTS users (username TEXT PRIMARY KEY, email TEXT UNIQUE, password TEXT, avatar TEXT DEFAULT 'https://cdn-icons-png.flaticon.com/512/149/149071.png');
...
io.on('connection', (socket) => {
    socket.on('register', async (d) => {
        try {
            ...
            socket.emit('auth_success', 'OK');
        } catch (e) { socket.emit('auth_error', 'Error'); }
    });
    ...
    socket.on('login', async (d) => {
        ...
        } else { socket.emit('auth_error', 'Fail'); }
    });
    ...
    socket.on('search_user', async (n) => {
        ...
        else socket.emit('auth_error', 'Not found');
    });
