...
db.connect().then(() => {
    db.query(`CREATE TABLE IF NOT EXISTS users (username TEXT PRIMARY KEY, email TEXT UNIQUE, password TEXT, avatar TEXT DEFAULT 'https://cdn-icons-png.flaticon.com/512/149/149071.png');
...
io.on('connection', (socket) => {
    ...
    socket.on('search_user', async (n) => {
        const r = await db.query("SELECT username, avatar FROM users WHERE username = $1", [n]);
        if (r.rows[0]) socket.emit('user_found', r.rows[0]);
    });
