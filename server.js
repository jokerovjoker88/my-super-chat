...
async function boot() {
    try {
        ...
        `);
        console.log("=== NEBULA UPDATED: EASY PASS & EYE ICON ===");
...
io.on('connection', (socket) => {
    socket.on('register', async ({ nick, email, pass }) => {
        // Упрощенная проверка: Минимум 6 символов, только буквы и цифры
        const passRegex = /^[A-Za-z\d]{6,}$/;
        if (!passRegex.test(pass)) {
            return socket.emit('auth_error', 'Пароль должен быть не менее 6 символов (буквы и цифры)');
        }

        const check = await db.query("SELECT * FROM users WHERE username = $1 OR email = $2", [nick, email]);
        if (check.rows.length > 0) return socket.emit('auth_error', 'Ник или почта уже заняты');
        ...
        const mailOptions = {
            ...
            to: email,
            subject: 'Код подтверждения Nebula Chat',
            html: `<h2 style="color:#40a7e3">Ваш код: ${code}</h2>`
        };

        transporter.sendMail(mailOptions, (err) => {
            if (err) return socket.emit('auth_error', 'Ошибка почты. Проверьте данные SMTP.');
    ...
    socket.on('verify_code', async ({ email, code }) => {
        ...
        if (user && user.code === code) {
            ...
            socket.emit('auth_success', 'Готово! Теперь войдите.');
        } else {
    ...
    socket.on('login', async ({ nick, pass }) => {
        ...
        if (!user || !(await bcrypt.compare(pass, user.password))) {
            return socket.emit('auth_error', 'Неверный ник или пароль');
        }
    ...
    socket.on('get_my_dialogs', async (me) => {
        const res = await db.query(`SELECT DISTINCT ON (partner) partner, 
